...
---
## 3.2 Criterion 2: Role in the Value Chain

Data submission obligations MAY vary depending on the role of the Data Provider within the battery value chain and the nature of the information held by the respective Data Provider.

The applicable data requirements, including mandatory and optional information, are defined according to the relevant Data Provider role and the applicable submodel. The detailed mapping of data submission obligations by supplier role is provided in (Appendix A – Data Submission Obligations based on Role in the Value Chain)[##7-appendix-a-data-submission-obligations-based-on-role-in-the-value-chain].

Data Providers SHALL provide the information applicable to their respective role as specified in the relevant requirements. Optional information MAY be provided where available and where agreed between the Data Provider and Data Consumer.
...
## 7 Appendix A Data Submission Obligations based on Role in the Value Chain
...
| TechnicalData.generalInformation | manufacturerName | mandatory | mandatory | mandatory | mandatory | mandatory |  |
| TechnicalData.generalInformation | companyLogo | optional | optional | optional | optional | optional | Optional, consistent with the corresponding attribute in the original Joint Battery Passport Model between Catena-X and IDTA |
| TechnicalData.generalInformation | manufacturerIdentifier | mandatory | mandatory | mandatory | mandatory | mandatory |  |
| TechnicalData.generalInformation | warrantyInformation | optional | mandatory | optional | mandatory | mandatory | Not applicable to all supplier types |
| TechnicalData.generalInformation | batteryCategory | optional | mandatory | optional | optional | optional | Applicable for battery manufacturers only. |
| TechnicalData.generalInformation | batteryMass | optional | mandatory | optional | mandatory | mandatory | Component Suppliers and Cell Producers MAY provide information on battery mass to enable Battery Manufacturers to determine and document the complete battery mass. |
| TechnicalData.generalInformation | productImages | optional | optional | optional | optional | optional | Optional, consistent with the corresponding attribute in the original Joint Battery Passport Model between Catena-X and IDTA |
| TechnicalData.technicalProperties | capacityEnergyVoltage | optional | mandatory | optional | optional | mandatory | Optional, as this information is not applicable to all products exchanged along the value chain. |
| TechnicalData.technicalProperties | roundTripEnergyEfficiency | optional | mandatory | optional | optional | mandatory | Optional, as this information is not applicable to all products exchanged along the value chain. |
| TechnicalData.technicalProperties | resistance | optional | mandatory | optional | optional | mandatory | Optional, as this information is not applicable to all products exchanged along the value chain. |
| TechnicalData.technicalProperties | powerCapability | optional | mandatory | optional | optional | mandatory | Optional, as this information is not applicable to all products exchanged along the value chain. |
| TechnicalData.technicalProperties | temperature | optional | mandatory | optional | optional | mandatory | Optional, as this information is not applicable to all products exchanged along the value chain. |
| TechnicalData.technicalProperties | lifetime | mandatory | mandatory | mandatory | mandatory | mandatory | The information MAY be provided for all products and SHALL be made available upon request. |
